...
from rlm_train.token_selection import TokenSelectionResult, choose_tokens
...
class CanonicalTrainer:
    ...
    def _execute_rollouts(
        ...
        rollouts = self.attempt_runner.run_many(record, count=count, mode="training")
        ...
        for objective, capability in capabilities.items():
            objective_selections = {
                rollout.rollout_id: choose_tokens(
                    ...
                    included_text=capability.token_scope,
                    student_id=self.policy_owner,
                )
                for rollout in rollouts
            }
